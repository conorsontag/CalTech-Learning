...
xs <- TRUE
iteration <- 0
store.temp <- data.frame(iter=0,pct=0)
while(xs){
	iteration <- iteration + 1
	wts <- pla2.eval(X,wts,Y)
	wts
	xs  <- !all(calcSign(X,wts) == Y)
	#cat("Iteration: ",iteration,"Pct",sum(calcSign(X,wts) == Y),"\n")
	store.temp[iteration,] <- c(iteration,sum(calcSign(X,wts) == Y))
}
iteration


# Now run this experiment 1000 times and take the average 
i <- 1
...
for(i in 1:100){
	...
	while(xs){
		iteration <- iteration + 1
		wts <- pla2.eval(X,wts,Y)
		xs  <- (!all(calcSign(X,wts) == Y) & (iteration < 150000))
	}
